...
// Mock data for pulling from job/quote
const mockJobLineItems = [
    { id: crypto.randomUUID(), description: 'Electrical Panel Upgrade', quantity: 1, unitPrice: 1500 },
    { id: crypto.randomUUID(), description: 'Outlet Installation (GFCI)', quantity: 4, unitPrice: 75 },
];
const mockQuoteLineItems = [
    { id: crypto.randomUUID(), description: 'Kitchen Lighting Fixture Installation', quantity: 3, unitPrice: 120 },
    { id: crypto.randomUUID(), description: 'Smart Thermostat Setup', quantity: 1, unitPrice: 150 },
];
...
const InvoiceFormContent = () => {
  ...
  useEffect(() => {
    if (jobIdFromQuery) {
      // Mock: find client related to job and prefill, then pull job items
      // const job = mockJobs.find(j => j.id === jobIdFromQuery); if you had mockJobs
      setClientId('1'); // Example: John Doe for job J001
      setLineItems(mockJobLineItems);
      console.log("Pre-filling invoice for Job ID:", jobIdFromQuery);
    } else if (quoteIdFromQuery) {
      // Mock: find client related to quote and prefill, then pull quote items
      setClientId('2'); // Example: Jane Smith for quote Q002
      setLineItems(mockQuoteLineItems);
      console.log("Pre-filling invoice for Quote ID:", quoteIdFromQuery);
    }
  }, [jobIdFromQuery, quoteIdFromQuery]);
  ...
  const handleSubmit = (e: FormEvent, status: string) => {
    e.preventDefault();
    const invoiceData = {
      invoiceId, clientId, invoiceDate, dueDate, lineItems, subtotal, taxRate, taxAmount, grandTotal,
      paymentsMade, amountDue, paymentTerms, notes, status,
    };
    console.log('Submitting Invoice:', invoiceData);
    alert(`Invoice ${status} (mock)!`);
  ...
  return (
    <form className='bg-white p-6 sm:p-8 rounded-lg shadow space-y-8'>
      ...
      <section className='grid grid-cols-1 md:grid-cols-3 gap-6'>
        ...
        <div>
          <label htmlFor='invoiceId' className='form-label flex items-center'><Hash size={14} className="mr-1.5 text-gray-400"/>Invoice ID</label>
          <input type='text' id='invoiceId' value={invoiceId} onChange={(e) => setInvoiceId(e.target.value)} className='default-input' />
        </div>
        <div>
          <label htmlFor='invoiceDate' className='form-label flex items-center'><CalendarDays size={14} className="mr-1.5 text-gray-400"/>Invoice Date</label>
        ...
        <div>
          <label htmlFor='dueDate' className='form-label flex items-center'><CalendarDays size={14} className="mr-1.5 text-gray-400"/>Due Date</label>
          <input type='date' id='dueDate' value={dueDate} onChange={(e) => setDueDate(e.target.value)} className='default-input' />
        </div>
      ...
      <section className='pt-6 border-t flex flex-wrap gap-3 justify-end'>
        <button type='button' onClick={(e) => handleSubmit(e, 'Draft')} className='btn-secondary'><Save size={18} className="mr-2" /> Save as Draft</button>
        <button type='button' onClick={() => alert('Recording payment (mock)...')} className='btn-secondary'><CreditCard size={18} className="mr-2" /> Record Payment</button>
        <button type='button' onClick={(e) => handleSubmit(e, 'Paid')} className='btn-secondary'><DollarSign size={18} className="mr-2" /> Mark as Paid</button>
        <button type='submit' onClick={(e) => handleSubmit(e, 'Sent')} className='btn-primary'><Send size={18} className="mr-2" /> Send Invoice</button>
      </section>
    </form>
  );
};
