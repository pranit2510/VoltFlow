...
const MarketingChannelsSection = () => {
  ...
  const [editFormData, setEditFormData] = useState<Partial<MarketingChannelData>>({});
  const [isAddingChannel, setIsAddingChannel] = useState(false);
  const [newChannelData, setNewChannelData] = useState<Partial<MarketingChannelData>>({
    channelName: '',
    cost: 0,
    leads: 0,
    jobs: 0,
  ...
  const calculateChannelMetrics = (channel: MarketingChannelData) => {
  ...
  // Calculate totals
  const calculateTotals = (): ChannelTotals => {
    const filteredData = channelData.filter(channel => channel.month === selectedMonth);
  ...
  const currentMonthData = channelData.filter(channel => channel.month === selectedMonth);
  ...
  // Edit handlers
  const handleEditStart = (channel: MarketingChannelData) => {
  ...
  const handleEditSave = () => {
    ...
    setChannelData(prev => prev.map(channel => 
      channel.id === editingRow 
        ? { ...channel, ...editFormData, month: selectedMonth }
        : channel
    ));
  ...
  const handleAddChannel = () => {
    if (!newChannelData.channelName) return;
    
    const newChannel: MarketingChannelData = {
      id: Date.now().toString(),
      channelName: newChannelData.channelName || '',
      cost: newChannelData.cost || 0,
      leads: newChannelData.leads || 0,
      jobs: newChannelData.jobs || 0,
      revenue: newChannelData.revenue || 0,
      month: selectedMonth
    };
    
    setChannelData(prev => [...prev, newChannel]);
    setNewChannelData({
      channelName: '',
      cost: 0,
      leads: 0,
  ...
  const handleDeleteChannel = (id: string) => {
    setChannelData(prev => prev.filter(channel => channel.id !== id));
  };
                    ...
                    <td className="px-4 py-4 whitespace-nowrap">
                      {isEditing ? (
                        <input
                          ...
                          onChange={(e) => setEditFormData(prev => ({ ...prev, channelName: e.target.value }))}
                ...
                <tr className="bg-blue-50 border-2 border-[#1877F2] border-dashed">
                  <td className="px-4 py-4">
                    <input
                      ...
                      onChange={(e) => setNewChannelData(prev => ({ ...prev, channelName: e.target.value }))}
